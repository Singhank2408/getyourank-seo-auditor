...
def run_speed(job_id):
    ...
    threading.Thread(target=run_pagespeed_batch, args=(job_id,api_key), daemon=True).start()
...
<aside>
  ...
  <div><button class="toggle-btn" onclick="toggleExtra()">▸ Page Speed API key</button>
    <div class="extra" id="extra" style="margin-top:8px">
      <label style="font-size:10px;color:#555;text-transform:uppercase;letter-spacing:.08em;font-weight:600">Google PSI API Key (optional)</label>
      <input type="text" id="apiKey" placeholder="AIza...">
      <span class="hint">Free key from Google Cloud Console. Without key: 25 URLs/day limit.</span>
    </div>
  </div>
...
<div class="panel" id="panel">
  ...
  <div class="rtab-c" id="rt-speed">
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px;flex-wrap:wrap">
      <button class="speed-btn" id="spdBtn" onclick="runSpeed()">▶ Run Speed Audit (first 20 URLs)</button>
      <span style="font-size:11px;color:#333">Google PageSpeed Insights API — enter API key in sidebar for scale.</span>
    </div>
    <div id="spd-prog" style="display:none;margin-bottom:14px">
      <div class="prog-hdr"><span class="prog-t" style="font-size:12px">Running PageSpeed...</span><span class="prog-c" id="sp-c">0/0</span></div>
      <div class="prog-bg"><div class="prog-fill" id="sp-f" style="width:0%"></div></div>
    </div>
    <div id="spd-done" style="display:none;font-size:12px;color:#444;padding:8px">Speed audit complete. Full CWV metrics per URL are in the Excel → Page Speed sheet.</div>
  </div>
...
function toggleExtra(){{
  ...
  document.querySelector('.toggle-btn').textContent=(el.classList.contains('show')?'▾':'▸')+' Page Speed API key';
}}
...
async function runSpeed(){{
  if(!jobId)return;
  document.getElementById('spdBtn').disabled=true;
  document.getElementById('spd-prog').style.display='block';
  await fetch('/api/speed/'+jobId,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{api_key:document.getElementById('apiKey').value.trim()}})}});
}}
