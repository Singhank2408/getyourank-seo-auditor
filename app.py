...
def run_speed_direct():
    ...
    results = {}
    for url in urls:
        results[url] = get_pagespeed(url, api_key)
        time.sleep(0.5)
...
<div class="panel" id="panel">
  ...
  <div class="rtab-c" id="rt-speed">
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px;flex-wrap:wrap">
      <button class="speed-btn" id="spdBtn" onclick="runSpeed()">▶ Run Speed Audit (up to 5 URLs)</button>
      <span style="font-size:11px;color:#333">Checks mobile CWV via Google PageSpeed Insights. Enter API key in sidebar.</span>
    </div>
...
async function runSpeed(){{
  ...
  if(!apiKey)return alert('Please enter a Google PageSpeed API key first.\n\nGet one free at console.cloud.google.com → APIs & Services → PageSpeed Insights API → Credentials.');
  ...
  try{{
    const res=await fetch('/api/speed/direct',{{
      ...
      body:JSON.stringify({{urls:auditedUrls,api_key:apiKey}})
    }});
    const data=await res.json();
    document.getElementById('spd-prog').style.display='none';
    btn.disabled=false;
    ...
    document.getElementById('spd-err').textContent='Request failed: '+e.message+'. Check your API key and try again.';
    document.getElementById('spd-err').style.display='block';
    document.getElementById('spd-prog').style.display='none';
    btn.disabled=false;
  }}
